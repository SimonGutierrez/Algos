...
function uniqueYesSum(input){
  let count = 0;

  for (let i = 0; i < input.length; i++) {
    let group = input[i];
    let uniqueResp = new Set();
    ...
    count += uniqueResp.size;
  }

  return count;
}

function yesToAllSum(input){
    let count = 0;

    for (let i = 0; i < input.length; i++) {
      let group = input[i];
      let uniqueResp = new Map();
      ...
      for (let j = 0; j < group.length; j++) {
        ...
        for (let k = 0; k < person.length; k++) {
          ...
          if (uniqueResp.has(resp)) {
           let newVal = uniqueResp.get(resp) + 1;
            uniqueResp.set(resp, newVal);
          } else {
             uniqueResp.set(resp, 1);
          }
        }
      }

      uniqueResp.forEach((value, key) => {
        if (value === group.length) count++;
      });
...
console.log('result', uniqueYesSum(parsed)); // 6430
console.log('result', yesToAllSum(parsed)); // 3125
